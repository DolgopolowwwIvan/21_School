...
// eq_matrix (сравнение)
// sum_matrix (сложение) есть
// sub_matrix (вычитание) есть
...
int check_dim(matrix_t* A, matrix_t* B);
...
int s21_sum_matrix(matrix_t* A, matrix_t* B, matrix_t* result) {
  ...
  if (check_dim(A, B) != 0 || check_dim(A, result) != 0 ||
      check_dim(result, B) != 0) {
    return 2;
  }

  for (int i = 0; i < A->rows; i++) {
    for (int j = 0; j < A->columns; j++) {
      // Возможно нужна проверка на что то отдельный элемент может быть NULL
...
int check_dim(matrix_t* A, matrix_t* B) {
  if (is_matrix_exists(A) != 0 || is_matrix_exists(B) != 0) {
    return 1;
  }
